...
;;(defconst json-beautify-command "python -c 'import sys,json; data=json.loads(sys.stdin.read()); print json.dumps(data,sort_keys=True,indent=4).decode(\"unicode_escape\").encode(\"utf8\",\"replace\")'")
(defconst json-beautify-command "jsonlint")
...
(defun beautify-json ()
  ...
  (let ((b (if mark-active (min (point) (mark)) (point-min)))
    ...
    (shell-command-on-region b e json-beautify-command (current-buffer) t)))
